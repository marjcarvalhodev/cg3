...
MyCamera::MyCamera(const glm::vec3 &pos, const glm::vec3 &tgt,
                   const glm::vec3 &upDir, float aspect)
    : position(pos), target(tgt), up(upDir), fov(45.0f), aspectRatio(aspect),
...
const glm::mat4 &MyCamera::getViewMatrix() const { return viewMatrix; }
